          ...
          {reports.map((r) => {
            ...
            return (
              <button
                ...
                className={cn("w-full text-left rounded-xl p-4 border transition-all",
                  active
              ...
              >
                <div className="flex items-start gap-3">
                  <div className={cn("h-10 w-10 rounded-lg grid place-items-center shrink-0",
                    r.format === "PDF" ? "bg-destructive/15 text-destructive" :
                    r.format === "JSON" ? "bg-accent/15 text-accent" : "bg-primary/15 text-primary"
                  )}>
                    <FileText className="h-4 w-4" />
                  </div>
                  <div className="min-w-0 flex-1">
                    <div className="font-medium text-sm truncate">{r.name}</div>
                    <div className="text-[11px] text-muted-foreground font-mono mt-0.5">{r.investigation} · {r.createdAt}</div>
        ...
        <Card className="glass border-border/60 overflow-hidden">
          <div className="flex items-center gap-2 p-4 border-b border-border/60">
            ...
            <div className="min-w-0">
              ...
              <div className="text-[11px] text-muted-foreground font-mono">{selected.investigation} · {selected.createdAt}</div>
            </div>
            <div className="ml-auto flex items-center gap-2">
              <Button variant="ghost" size="sm" className="gap-1"><Eye className="h-3.5 w-3.5" /> Preview</Button>
            ...
            <div className="max-w-2xl mx-auto bg-white text-black rounded-md p-10 shadow-2xl aspect-[8.5/11]">
              <div className="flex items-center justify-between border-b pb-4">
                <div>
                  ...
                  <div className="mt-1 text-xl font-bold">{selected.name}</div>
                </div>
                <div className="text-right">
                  <div className="text-[10px] uppercase tracking-widest text-slate-400">AXIOM Intel</div>
                  <div className="text-[10px] font-mono text-slate-500">{selected.investigation}</div>
                </div>
              </div>
              <div className="mt-6 space-y-4 text-xs text-slate-700">
                ...
                <div className="border-t pt-3 text-[9px] text-slate-400">
                  Page 1 of {selected.pages || 1} · Generated {selected.createdAt}
                </div>
              </div>
            </div>
          </div>
